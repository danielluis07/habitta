...
The scene opens with only `/models/district-low.glb`, with the bundled Three.js Meshopt decoder, the base environment map (see [Rendering quality tiers](#rendering-quality-tiers)) and the [textured ground](#textured-ground)'s maps. Selection targets and label anchors come from the collection's [runtime bindings](runtime-assets.md). Camera framing follows the model bounds, so replacement exports do not require per-building camera coordinates. `components/district-scene/framing.ts` fits the corners of each framed building, plus the space above its label, to the view from the southeast: the default overview lets the three buildings fill 85% of the limiting axis, centred; a selected building fills 60%, keeping some district around it.

**Fixed camera.** [#22](https://github.com/danielluis07/habitta/issues/22) fixes the camera ([ADR 0001](adr/0001-fixed-camera-stage-set-district.md)). The visitor sees only the district overview and each building's view. No drag, wheel, pinch or key moves the camera, and there are no rotate, zoom or Reset view controls. It moves only by the flight that follows a selection; closing the building overview flies back to the district overview, clears the selection and updates the URL. The wordmark does the same from anywhere in the journey. The canvas takes clicks on buildings and nothing else, so the landscape needs detail only where these views look (see [Overview near band](#overview-near-band)).

[#38](https://github.com/danielluis07/habitta/issues/38) made the district a full-viewport backdrop with the header and arrival copy laid over its sky. The page tells the scene its **clearance**: how far its own layers cover each canvas edge. Over the overview, that's the header and arrival copy. Beside a building, it's the header plus the 420px overview panel on wide screens, or the header plus the 60% bottom sheet on narrow ones. Framing also keeps 52px clear below the page's layers for the labels. The world-space room above each anchor shrinks with distance, so on wide, short windows (1536 × 730, for example) it alone let the middle label land exactly on the copy's edge. The buildings fill their share of the clear region, and a lens shift (`setViewOffset`) moves the camera's axis to the region's centre. That shift slides the image without tilting the camera, so verticals stay upright, and it flies with the camera. The overview looks down about 6° instead of the building views' 18°, so the sky and a thin band of hazy ranges, not treed ground, sit behind the copy. It also keeps the bottom quarter of its clear region below the buildings for the nearer landscape that frames them, like the foreground of an architectural render: the buildings fill 85% of the region above that band. Labels under the page's layers are hidden and leave the tab order, like offscreen ones. Labels are ordinary DOM buttons projected from the anchors on each rendered frame; mesh descendants also accept pointer selection. Offscreen labels leave the tab order, and the canvas itself has no tab stop.
...
The overview's foreground is the ground the textured-ground and vegetation work should target. Nothing within 60 m of the overview camera is in view: the camera looks down the district from above the head of the valley, 75–185 m above the ground below it, and its view reaches the ground only about 180 m out. So the near band is the ground the foreground band shows, nearer than every building:

- **Where:** from about the lane (z ≈ 10 m) south across the district's southern half and over its edge, to the head of the valley (z ≈ 140–215 m); from about 105 m west to 270 m east of the district centre. Contour's and Grove's olive groves and terrace walls, and the scrub, cypresses and rocks beyond the district's southern edge, sit in it today. The vegetation and rocks take their full versions here and in the district, and lighter ones beyond ([runtime-assets.md](runtime-assets.md#vegetation-and-rocks)).
- **How far:** 180–240 m from the camera on a 1440 × 1000 desktop, 320–410 m on wide, short desktops (1280 × 720, 1536 × 730), and 400–550 m on a 390 × 844 phone (520–640 m at 320 × 800). The camera backs off on narrower, shorter canvases to fit the three buildings, so the phone's foreground is farther and smaller.

These figures come from the placeholder terrain; ground reshaped under the foreground changes them. `tests/district-atmosphere.test.ts` checks that the bottom of the overview shows ground nearer than the buildings on desktop and mobile.

## Textured ground

[#49](https://github.com/danielluis07/habitta/issues/49) gives the near ground dry grass, bare earth and rock, and the lane, paths and bench tops compacted gravel between stone kerbs. The asset side, its sources and licences are in [runtime-assets.md](runtime-assets.md#textured-ground). `components/district-scene/ground.ts` holds the runtime side:

- **Loading.** The eight 512 px maps load with the district and the base environment map, suspending with them, so a failed map opens simple view like any other scene asset. Both tiers use them. They tile with anisotropic filtering up to 8×, because the overview sees the ground at grazing angles.
- **Shader.** Meshes whose geometry carries layer weights get a patch on three.js's standard material (`onBeforeCompile`, one program for all three ground materials). It multiplies the vertex colour by the weighted maps, each divided by its token, blends the layers' normals and roughness, and fades all of it out between 700 m and 1,000 m from the camera. Each colour map is sampled at two scales, turned against each other, so no repeat shows from the overview or any building view. Gradients are taken once, outside the branches that skip absent layers, so skipping costs no filtering seams. A layer only costs samples where its weight is above zero; most of the ground has one or two.
- **Cost.** Up to three texture samples per present layer, eight texture units in all, and no extra draws or triangles.

## Vegetation and rocks

[#50](https://github.com/danielluis07/habitta/issues/50) gives the landscape olives, cypresses and scrub with leaf-card foliage on real branches, and scanned limestone rocks. The assets, their sources and budgets are in [runtime-assets.md](runtime-assets.md#vegetation-and-rocks). They arrive embedded in `district-low.glb`, so they load and fail with it and add nothing to load. The runtime side is small, in `shade()` in `components/district-scene/models.ts`:

- **One normal on both faces.** three.js turns a double-sided surface's normal around on its back face. A leaf card's normals lean outward from its crown instead, so the scene keeps them on both faces of every alpha-tested, double-sided material: its fragment shader undefines `DOUBLE_SIDED` before `normal_fragment_begin`, the only place, with no normal map, that uses it. Without it, the half of each crossed card seen from behind lit as if it faced into the crown, and cypresses turned nearly black.
- **Cut-out shadows.** Leaf cards cast shadows like any opaque mesh. three.js's shadow pass takes their map and cut-off, so on the high tier the shadow is the leaves', not the cards'.
- **Rocks** need nothing at runtime: their tangents come with the model.
...
Seam 1 covers scene-label selection, keyboard focus return, leaving the scene by Tab, viewpoint capture through the index and browser history, preservation through stories/building switches, direct links, and reduced-motion input. The full suite contains 208 tests. `tests/rendering-quality.test.ts` covers tier settings, selection from device capability and the renderer, step-down on a sustained low frame rate (not a hitch or a hidden tab), and which materials reflect, cast and receive, without WebGL. `tests/district-atmosphere.test.ts` covers the fixed camera's pitches, the default overview framing on desktop and mobile, framing into the region the page leaves clear above the overview's foreground with every label clear of the arrival copy on wide, short desktops, the ground filling that foreground, and that the ground outruns the haze from every framed, lens-shifted view, without WebGL; `tests/runtime-assets.test.ts` checks that the committed props are instanced and that the ground textures count toward the opening transfer; `tests/textured-ground.test.ts` checks the ground maps' palette, seams and CC0 sources, the layer weights in the district, and the shader patch against three.js's standard material; `tests/vegetation-and-rocks.test.ts` checks the leaf cards, branches and atlas, the near and far versions and where they stand, the rocks' maps and tangents, and the sources. `tests/rendering-quality.test.ts` also checks that leaf cards cast their cut-out and keep their normals on both faces, and `tests/district-atmosphere.test.ts` that every fixed view sees only the finely gridded ground. The production build also runs Khronos validation, binding checks, and asset budgets.
...
The fixed-camera review ([#22](https://github.com/danielluis07/habitta/issues/22)) used headless Chromium with SwiftShader against the dev server, at 1440 × 1000 and at 390 × 844 with touch. It wrapped WebGL draw calls and compared canvas captures:

- Mouse drag (left and right button), ten wheel turns, a one-finger swipe, a two-finger pinch out and in, and arrow, +, −, Page Up/Down, Home, End and WASD keys drew 0 calls; the canvas was unchanged and the page didn't scroll.
- Selecting Crest flew to its unchanged building view; Return to district flew back to the arrival overview, differing from the first capture in under 0.01% of pixels by at most 7 levels of 255. Every overview-to-building flight stays at least 59 m above the ground. No console errors or warnings.
- The overview at 1440 × 1000, 1536 × 730 and 390 × 844 showed the buildings above a band of nearer ground, with olives, a cypress and scrub at its corners, and the ranges as a thin band on the horizon. All labels were visible and clear of the copy.
- Contrast was measured as in the #38 review. At the default framing, from 320 × 800 to 1920 × 1080, the heading held at least 11.7:1, the intro 12.0:1, the wordmark 11.4:1 and the eyebrow 4.87:1. The camera now faces higher into the sky dome, where the zenith is a little darker; measured the same way, the 12° overview gave 11.9, 12.3, 11.5 and 4.96. Short viewports and 200% and 400% zoom (390 × 664, 320 × 568, 720 × 450, 360 × 225, 320 × 180) still stack the copy above the district on paper, with no horizontal scrolling.
- Building views are unchanged. In Grove's view on a 390 × 844 phone, the wordmark sits over the olive grove at as little as 1.7:1; the 12° build measures the same, so it predates this change.

The textured-ground review ([#49](https://github.com/danielluis07/habitta/issues/49)) used headless Chrome on a physical GPU (AMD Radeon Vega 10 through ANGLE's Direct3D 11 backend) against the dev server, at 1440 × 1000 and at 390 × 844 with touch, on both tiers (`?quality=base`, `?quality=high`). It captured the overview and each building view, and the same views before the change:

- Both tiers fetched the district, `highland-512.hdr` and the eight ground maps; only the high tier then fetched `highland-1024.hdr`. No console errors or warnings on any run. A 404 for one ground map opened simple view with the asset-failure notice.
- The overview's foreground read as dry grass with earth and small stones showing through it, and rock where the colour turns to rock, instead of smooth colour. No repeat showed in the overview or in any building view, including 1:1 crops of the nearest ground. On the phone the near band is 400–550 m away, so the texture is finer there, but still reads.
- The ground kept its palette: over the same foreground regions, the mean colour moved by at most 2 of 255 per channel, while its local variation rose by about half.
- The lane read as surfaced gravel between pale kerbs, with the darker gutter and the stone edge below the kerb as lines on either side, in every building view. From the desktop overview the lane is 8 to 10 px wide, so its kerbs are sub-pixel; there it reads as a pale surfaced strip with crisp edges.
- Frame times during the flight to Crest, motion on, median of each flight over three flights: base tier 5.2–6.9 ms (7.0–8.7 ms before), high tier 9.0–11.3 ms (10.4–17.1 ms before). The difference is within run-to-run noise; the textured ground adds no measurable cost on this GPU. Physical-phone profiling is still required, as above.

The vegetation-and-rocks review ([#50](https://github.com/danielluis07/habitta/issues/50)) used headless Chrome on the same physical GPU against the dev server, at 1440 × 1000 and at 390 × 844 with touch, on both tiers. It captured the overview and each building view before and after, and rendered each prop up close on a temporary review page:

- No console errors or warnings on any run.
- Near olives read as olives: a split trunk and limbs under a silvery crown of sprays, with trunks and limbs showing between the crowns of Contour's and Grove's groves. Cypresses read as green flames and scrub as leafy mounds. Far plants read as whole trees and bushes from the overview. From the Crest view, which sees the ridge north of the district 150–250 m away, far olives read as round crowns, though flatter than near ones.
- On the high tier, olives, cypresses and scrub cast leafy shadows northwest, with no acne or peter-panning in 3× crops of the overview and building views. The base tier draws no shadows, as before (#48).
- Rocks are at most about 15 px wide in any fixed view. Up close, the near rock shows the scan's cracked and pitted limestone, lit from the right side; the far rock, meant for 300 m and more, is coarse.
- Frame times during the flight to Crest, motion on, median of each flight, measured in one session: base tier 5.3–8.7 ms over six flights (4.9–6.3 ms over three before), high tier 9.1–16.3 ms (8.5–13.6 ms before). The ranges overlap, but the leaf cards may cost a little; physical-phone profiling (#26) is still required.
