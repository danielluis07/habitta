...
| `components/district-scene/` | Client-side Three.js scene, camera framing, model loading, atmosphere, and display controls. `quality.ts` defines the rendering tiers, their selection and step-down. |
...
| `scripts/` | Model and environment-map generation, and their validation, run with Bun. `scripts/placeholder-models/` draws the development placeholders' buildings, landscape and props. |
